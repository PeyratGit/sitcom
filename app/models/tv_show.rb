class TvShow < ApplicationRecord
  ...
  def popularity_score(user)
    ...
    tv_show_feedbacks_from_followings = Feedback.where('user_id in (?)', followings).where(tv_show: self)
    ...
    like_score = numerator.fdiv(denominator) * 10

    rating = 0
    index = 0
    tv_show_feedbacks_from_followings.each do |feedback|
      rating += feedback.rating.nil? ? 0 : feedback.rating
      index += feedback.rating.nil? ? 0 : 1
    end
    average_rating = index.positive? ? rating.fdiv(index) * 2 : 0
    ...
    watched_score = tv_show_watched_from_followings.fdiv(count) * 10

    if tv_show_feedbacks_from_followings.nil? || tv_show_watched_from_followings.zero?
      return 0.0
    elsif average_rating.zero?
      return ((0.7 * like_score) + (0.3 * watched_score)).round(1)
    else
      return ((0.6 * like_score) + (0.2 * average_rating) + (0.2 * watched_score)).round(1)
    end
  end
end
